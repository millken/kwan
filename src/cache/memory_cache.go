...
//Do command
func (bc *MemoryCache) Do(cmd string, args ...interface{}) (reply interface{}, err error) {
	switch cmd {
	case "keys":
		return bc.cmd_keys(args[0].(string)), nil
	default:
		return nil, errors.New("unknow cmd")
	}
}

func (bc *MemoryCache) cmd_keys(key string) map[string]interface{}  {
	result := make(map[string]interface{})
	for name, _ := range bc.items {
		fmt.Println(name)
		val := bc.Get(name)
		if strings.Index(name, key) != -1 && val != nil {
			result[name] = val
		}
	}
	return result
